...
namespace Battle
{
    ...
    public class BattleObj : MonoBehaviour
    {
        ...
        private IEnumerator HealProcess(BattleUnit target, float healHP, float healSolNum)
        {
            ...
            var damageDisplay = (target.IsPlayer ? mDamageDisplayPlayer : mDamageDisplayEnemy).GetComponent<DamageDisplay>();
            damageDisplay.Display(System.Math.Max(healHP, healSolNum), false);
            ...
            else
            {
                ...
                foreach (var unit in targetsList)
                {
                    ...
                    if (preUnit != null)
                    {
                        ...
                        unit.Face.SetSelectFlame(false);
                    }
                    preUnit = unit;
                    // ターゲットユニットをスライドインさせる
                    yield return unit.SlideIn();
        ...
        private IEnumerator CoTurnProcess()
        {
            ...
            else
            {
                // 次のターン
                Music.PlayNextTurn();
                //				if (TurnNum == Area.AreaData.Time - 1)
                //					yield return StartCoroutine(CoSkillAction(OrderController.OrderDiplayObj[1].BattleUnit.IsPlayer?EnemyUnits[0]:PlayerUnits[0],
                //						null, mGame.SkillData[21], OrderController.OrderDiplayObj[1].BattleUnit.IsPlayer, false));
                StartCoroutine("CoTurnProcess");
            }
        }
