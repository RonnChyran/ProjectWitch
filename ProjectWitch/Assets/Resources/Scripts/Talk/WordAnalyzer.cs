...
namespace Script.Analyzer{
	...
	public class WordAnalyzer
	{
		...
		//字句解析をする
		public List<WordWithName> Analyze(string text, out ErrorList errorList)
		{
			...
			List<WordWithName> words = new List<WordWithName>();
			...
			{//行ごとに字句解析を実施、前方から順に読む
				for (int currLine = 0; currLine < lines.Count; currLine++)
				{
					...
					{
						ErrorInfo oneErrorInfo;
						WordWithName word = AnalyzeWord (textLine, currIndex, currLine, out oneErrorInfo);
						words.Add (word);
		...
		//実際に字句解析をしてる部分
		private WordWithName AnalyzeWord(string text, int currIndex, int currLine, out ErrorInfo errorInfo)
		{
			errorInfo = null;
			//字句解析部
			...
			if (new Regex ("^Invalid_").Match (word.Name).Success) {
				errorInfo = new ErrorInfo(word.Line ,word.Index ,"無効な文字列を検出しました(" + word.Word + ")。" );
			}
			return word;
		}
	}
